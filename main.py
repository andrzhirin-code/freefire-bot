...
def handle_message(user_id, text, ref=None):
    ...
    if data[key].get("banned"):
        send_message(user_id, "⛔ Ты заблокирован. Если считаешь что это ошибка — свяжись с админом.")
        return
    ...
    if t == "🎯 Гайды":
        ...
        kb = {
            "one_time": False,
            "buttons": [
                [{"action": {"type": "text", "label": "🎛 Настройка раскладки (HUD)"}, "color": "primary"}],
                [{"action": {"type": "text", "label": "🎯 Чувствительность"}, "color": "primary"}],
                [{"action": {"type": "text", "label": "📳 Гироскоп"}, "color": "primary"}],
    ...
    if t == "🔧 Решение проблем":
        ...
        kb = {
            ...
            "buttons": [
                ...
                [{"action": {"type": "text", "label": "🎯 Не могу попасть в голову"}, "color": "primary"}],
                [{"action": {"type": "text", "label": "📊 Сенса слетела"}, "color": "primary"}],
    ...
    if t == "💡 Игровые фишки":
        kb = {
            "one_time": False,
            "buttons": [
                [{"action": {"type": "text", "label": "🎯 Свайп в голову (Drag Headshot)"}, "color": "primary"}],
    ...
    if t == "🎛 Настройка раскладки (HUD)":
        send_message(user_id,
            "🎛 РАСКЛАДКА КНОПОК (HUD)\n\n"
            "2 пальца:\n▸ Кнопка огня — справа внизу\n▸ Размер кнопок: 140-160%\nДля новичков и маленьких экранов.\n\n"
            "3 пальца:\n▸ Кнопка огня — слева вверху\n▸ Прицел — справа внизу\n▸ Размер кнопок: 135-155%\nПереходный хват.\n\n"
            "4 пальца:\n▸ Кнопка огня — слева вверху или по центру\n▸ Размер кнопок: 125-145%\nУниверсальный хват.\n\n"
            "5 пальцев:\n▸ Две кнопки огня\n▸ Размер кнопок: 115-135%\nДля продвинутых.\n\n"
            "6 пальцев:\n▸ Две кнопки огня\n▸ Размер кнопок: 105-125%\nДля мощных телефонов.\n\n"
            "📱 Планшеты:\n▸ Кнопки крупнее (150-190%)\n▸ Разноси кнопки дальше к краям\n\n"
            "💡 Не меняй раскладку первые 5 дней — мышцы должны привыкнуть.\n\n"
            "🔥 Хочешь персональную раскладку под свой телефон?",
            keyboard=premium_inline_get_kb())
        return

    if t == "🎯 Чувствительность":
        send_message(user_id,
            "🎯 ЧУВСТВИТЕЛЬНОСТЬ (СЕНСА) — КАК НАСТРОИТЬ\n\n"
            "Сенса — это скорость поворота камеры когда ты водишь пальцем.\n"
            "▸ Высокая = быстрый поворот, но труднее целиться\n"
            "▸ Низкая = легче целиться, но медленный поворот\n\n"
            "В игре 6 параметров (Настройки → Чувствительность):\n\n"
            "🔵 ОБЗОР — скорость поворота когда смотришь по сторонам\n"
            "🟢 КОЛЛИМАТОР — когда используешь красную точку\n"
            "🟡 2x — когда используешь увеличение 2x\n"
            "🟠 4x — когда используешь увеличение 4x\n"
            "🔴 СНАЙПЕРСКИЙ — когда используешь AWM\n"
            "⚪ СВОБОДНЫЙ ОБЗОР — скорость осмотра без прицеливания\n\n"
            "💡 ГЛАВНОЕ ПРАВИЛО:\n"
            "Проведи пальцем от края до края экрана — должен быть полный разворот на 180°.\n\n"
            "📊 ПОРЯДОК НАСТРОЙКИ:\n"
            "1. DPI → 2. Обзор → 3. Коллиматор → 4. 2x → 5. 4x → 6. Снайперский → 7. Свободный обзор\n"
            "Меняй по одному параметру за раз, тестируй 10-15 минут.\n\n"
    ...
    if t == "📳 Гироскоп":
        send_message(user_id,
            ...
            "3. Не пытайся целиться только гироскопом\n\n"
            "Ошибки:\n"
            "▸ Экран трясёт → уменьши на 10-15\n"
            "▸ Не чувствуешь → увеличь на 5-10\n\n"
            "🔥 Хочешь точные настройки гироскопа под себя?",
            keyboard=premium_inline_get_kb())
        return

    if t == "🎯 Свайп в голову (Drag Headshot)":
        send_message(user_id,
            "🎯 СВАЙП В ГОЛОВУ (DRAG HEADSHOT)\n\n"
            "Это когда ты стреляешь и одновременно ведёшь пальцем вверх — пуля летит в голову.\n\n"
            "Как делать правильно:\n"
            "▸ Наведись на грудь врага\n"
            "▸ Нажми кнопку огня\n"
            "▸ Резко проведи пальцем вверх (1-2 см)\n"
            "▸ Отпусти\n\n"
            "Весь свайп занимает 0.1-0.2 секунды.\n\n"
            "3 главные ошибки:\n"
            "❌ Слишком длинный свайп (через весь экран)\n"
            "❌ Целишься сразу в голову (начинай с груди)\n"
            "❌ Плавно ведёшь палец (нужно резко)\n\n"
            "Упражнение:\n"
            "▸ Тренировка с Desert Eagle / M1887\n"
            "▸ 5 минут только хедшотов перед игрой\n\n"
            "🔥 Нужна точная сенса для хедшотов?",
            keyboard=premium_inline_get_kb())
        return
    ...
    if t == "🌐 Высокий пинг":
        send_message(user_id,
            ...
            "1. Проверь интернет:\n   Домашний Wi-Fi может быть загружен → переключись на 4G/5G\n\n"
            "2. Сядь ближе к роутеру — чем дальше, тем хуже сигнал\n\n"
            "3. Выбери сервер вручную:\n   Настройки игры → Сервер → Ближний Восток\n\n"
            "4. Перезагрузи роутер:\n   Выключи на 30 секунд, включи обратно\n\n"
            "5. Отключи загрузки — никаких торрентов и обновлений\n\n"
    ...
    if t == "❌ Не заходит в игру":
        send_message(user_id,
            ...
            "6. Проверь дату и время → Автоматически\n\n"
    ...
    if t == "🔥 Греется телефон":
        send_message(user_id,
            "🔥 ГРЕЕТСЯ ТЕЛЕФОН\n\n"
            "1. Сними чехол во время игры — он не даёт теплу выходить\n\n"
            "2. Не играй на зарядке — заряди до игры\n\n"
            "3. Снизь графику:\n   Качество → Плавная | FPS → Средний\n\n"
            "4. Включи режим «Не беспокоить» — отключает фоновые уведомления\n\n"
            "5. Закрой все приложения перед игрой\n\n"
            "6. Дай телефону остыть — перерыв 10-15 минут\n\n"
            "7. Кулер для телефона стоит 300-500₽ — реально помогает\n\n"
            "🔥 Хочешь точные настройки чтобы телефон не грелся?",
            keyboard=premium_inline_get_kb())
        return

    if t == "🎯 Не могу попасть в голову":
        send_message(user_id,
            "🎯 НЕ МОГУ ПОПАСТЬ В ГОЛОВУ\n\n"
            "Самая частая проблема! Вот что проверь:\n\n"
            "1. Сенса слишком высокая — прицел перелетает\n   → Уменьши Обзор на 3-5 пунктов\n\n"
            "2. Сенса слишком низкая — не дотягиваешь\n   → Увеличь Обзор на 3-5 пунктов\n\n"
            "3. Кнопка огня слишком маленькая\n   → Увеличь до 65-70%\n\n"
            "4. Делаешь длинный свайп — нужно короткий (1-2 см)\n\n"
            "5. Не используешь гироскоп для доводки\n\n"
            "🔥 Нужна точная сенса для хедшотов?",
            keyboard=premium_inline_get_kb())
        return
    ...
    if t == "📊 Сенса слетела":
        send_message(user_id,
            "📊 СЕНСА СЛЕТЕЛА ПОСЛЕ ОБНОВЛЕНИЯ\n\n"
            "Частая проблема после обновлений Free Fire.\n\n"
            "Что делать:\n"
            "1. Проверь DPI в настройках разработчика — мог сброситься\n\n"
            "2. Проверь графику — после обновления могла включиться Ультра\n\n"
            "3. Очисти кэш игры\n\n"
            "4. Если играешь на FF MAX — после обновления сенса ощущается медленнее\n   → Увеличь общую сенсу на 5-10 пунктов\n\n"
            "5. Настрой сенсу заново по нашему гайду\n\n"
            "🔥 Получи персональную настройку и не парься!",
            keyboard=premium_inline_get_kb())
        return
    ...
    if t == "💣 Лучшие позиции":
        send_message(user_id,
            ...
            "Некстерра:\n▸ Twin Bridges\n▸ Deca Square\n▸ Museum\n\n"
            "Пургаторий:\n▸ Центральная башня\n▸ Мосты\n\n"
            "💡 Всегда проверяй позицию дроном или гранатой.\n\n"
    ...
    if t == "🔫 Разбор оружия":
        send_message(user_id,
            "🔫 РАЗБОР ОРУЖИЯ (2026)\n\n"
            "M4A1 / SCAR — средняя отдача, контроль очереди\n"
            "AK47 / Groza — сильная отдача, высокий урон\n"
            "MP40 / UMP — ближний бой, стрейфы\n"
            "MAG-7 / M1887 — один выстрел, выстрел + прыжок\n"
            "AWM / SVD — дальняя дистанция, быстрый скоп\n"
            "Woodpecker / XM8 — точный, контроль очереди\n\n"
    ...
    if t == "❌ Частые ошибки":
        send_message(user_id,
            "❌ ЧАСТЫЕ ОШИБКИ\n\n"
            "1. Слишком высокая сенса — прицел перелетает\n   → Уменьши обзор на 3-5 пунктов\n\n"
            "2. Кнопка огня далеко — не успеваешь нажать\n   → Подвинь ближе к центру\n\n"
            "3. Играешь на зарядке — телефон греется, FPS падает\n\n"
            "4. Меняешь сенсу каждый день — мышцы не привыкают\n   → Играй с одной сенсой минимум неделю\n\n"
            "5. Копируешь сенсу блогера — у него другой телефон\n\n"
            "6. Не используешь тренировочный режим\n   → 10 минут перед игрой улучшат точность\n\n"
    ...
    if t in ["🔥 ПРЕМИУМ НАСТРОЙКА — 99₽", "🔥 Хочу премиум"]:
        send_message(user_id,
            ...
            "✅ Раскладку под твои пальцы и размер экрана\n"
